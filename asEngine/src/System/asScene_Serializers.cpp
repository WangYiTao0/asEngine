...
namespace as
{
	...
	namespace asScene
	{
		...
		void NameComponent::Serialize(asArchive& archive, uint32_t seed)
		{
		...
		void LayerComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> layerMask;
		...
		void TransformComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void PreviousFrameTransformComponent::Serialize(asArchive& archive, uint32_t seed)
		{
		...
		void HierarchyComponent::Serialize(asArchive& archive, uint32_t seed)
		{
		...
		void MaterialComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			...
			if (archive.IsReadMode())
			{
				...
				{
					baseColorMap = asResourceManager::Load(dir + baseColorMapName);
				}
				if (!surfaceMapName.empty())
				{
					surfaceMap = asResourceManager::Load(dir + surfaceMapName);
				}
				if (!normalMapName.empty())
				{
					normalMap = asResourceManager::Load(dir + normalMapName);
				}
				if (!displacementMapName.empty())
				{
					displacementMap = asResourceManager::Load(dir + displacementMapName);
				}
				if (!emissiveMapName.empty())
				{
					emissiveMap = asResourceManager::Load(dir + emissiveMapName);
				}
				if (!occlusionMapName.empty())
				{
					occlusionMap = asResourceManager::Load(dir + occlusionMapName);
				}
		...
		void MeshComponent::Serialize(asArchive& archive, uint32_t seed)
		{
		...
		void ImpostorComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void ObjectComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void RigidBodyPhysicsComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void SoftBodyPhysicsComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void ArmatureComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void LightComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			std::string dir = archive.GetSourceDirectory();
			...
			if (archive.IsReadMode())
			{
				...
				for (size_t i = 0; i < lensFlareNames.size(); ++i)
				{
					...
					{
						lensFlareRimTextures[i] = asResourceManager::Load(dir + lensFlareNames[i]);
					}
				}
		...
		void CameraComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void EnvironmentProbeComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void ForceFieldComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void DecalComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void AnimationComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			if (archive.IsReadMode())
			{
				archive >> _flags;
		...
		void WeatherComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			std::string dir = archive.GetSourceDirectory();
			...
			if (archive.IsReadMode())
			{
				...
				{
					archive >> skyMapName;
					skyMap = asResourceManager::Load(dir + skyMapName);
				}
		...
		void SoundComponent::Serialize(asArchive& archive, uint32_t seed)
		{
			std::string dir = archive.GetSourceDirectory();
			...
			if (archive.IsReadMode())
			{
				...
				if (!filename.empty())
				{
					soundResource = asResourceManager::Load(dir + filename);
					asAudio::CreateSoundInstance(soundResource->sound, &soundinstance);
				}
		...
		void Scene::Serialize(asArchive& archive)
		{
			if (archive.IsReadMode())
			{
				uint32_t reserved;
			...
			uint32_t seed = (uint32_t)asRandom::getRandom(1, INT_MAX);
		...
		Entity Scene::Entity_Serialize(asArchive& archive, Entity entity, uint32_t seed, bool propagateSeedDeep)
		{
