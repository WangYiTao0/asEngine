...
namespace as
{
	namespace asECS
	{
		...
		{
			return asRandom::getRandom(1, INT_MAX);
		}
	...
	//	seed : ensures that entity will be unique after loading (specify seed = 0 to leave entity as-is)
		inline void SerializeEntity(asArchive& archive, Entity& entity, uint32_t seed)
		{
		...
		public:
			...
			inline void Serialize(asArchive& archive, uint32_t seed = 0, bool propagateSeedDeep = true)
			{
				if (archive.IsReadMode())
				{
