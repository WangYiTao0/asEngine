...
namespace as
{
	...
	namespace asScene
	{
		...
		const Texture* MaterialComponent::GetBaseColorMap() const
		{
			...
			return asTextureHelper::getWhite();
		}
		...
		void MeshComponent::CreateRenderData()
		{
			...
			// vertexBuffer - POSITION + NORMAL + SUBSETINDEX:
			{
				...
				for (size_t i = 0; i < vertices.size(); ++i)
				{
					...
					_min = asMath::Min(_min, pos);
					_max = asMath::Max(_max, pos);
				}
		...
		void Scene::Update(float dt)
		{
			...
			asJobSystem::Wait(ctx); // dependecies
		...
		Entity Scene::Entity_Duplicate(Entity entity)
		{
			...
			uint32_t seed = asRandom::getRandom(1, INT_MAX);
		...
		)
		{
			...
			if (!textureName.empty())
			{
				...
				material.baseColorMap = asResourceManager::Load(material.baseColorMapName);
			}
			if (!normalMapName.empty())
			{
				material.normalMapName = normalMapName;
				material.normalMap = asResourceManager::Load(material.normalMapName);
			}
		...
		)
		{
			...
			sound.soundResource = asResourceManager::Load(filename);
			asAudio::CreateSoundInstance(sound.soundResource->sound, &sound.soundinstance);
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)prev_transforms.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		void RunTransformUpdateSystem(
			...
			ComponentManager<TransformComponent>& transforms)
		{
			asJobSystem::Dispatch(ctx, (uint32_t)transforms.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), 1, [&](asJobDispatchArgs args) {
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)materials.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)impostors.GetCount(), 1, [&](asJobDispatchArgs args) {
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)cameras.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		)
		{
			...
			asJobSystem::Dispatch(ctx, (uint32_t)decals.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		)
		{
			...
			asJobSystem::Dispatch(ctx, (uint32_t)probes.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		{
			asJobSystem::Dispatch(ctx, (uint32_t)forces.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
		...
		)
		{
			...
			asJobSystem::Dispatch(ctx, (uint32_t)lights.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {
				...
				case LightComponent::DIRECTIONAL:
					aabb.createFromHalfWidth(asRenderer::GetCamera().Eye, XMFLOAT3(10000, 10000, 10000));
					break;
		...
		void RunParticleUpdateSystem(
			...
			const ComponentManager<TransformComponent>& transforms,
			const ComponentManager<MeshComponent>& meshes,
			ComponentManager<asEmittedParticle>& emitters,
			ComponentManager<asHairParticle>& hairs,
			float dt
		)
		{
			asJobSystem::Dispatch(ctx, (uint32_t)emitters.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {

				asEmittedParticle& emitter = emitters[args.jobIndex];
				Entity entity = emitters.GetEntity(args.jobIndex);
				const TransformComponent& transform = *transforms.GetComponent(entity);
				emitter.UpdateCPU(transform, dt);
				});

			asJobSystem::Dispatch(ctx, (uint32_t)hairs.GetCount(), small_subtask_groupsize, [&](asJobDispatchArgs args) {

				asHairParticle& hair = hairs[args.jobIndex];
		...
		)
		{
			...
			asAudio::SoundInstance3D instance3D;
			...
			for (size_t i = 0; i < sounds.GetCount(); ++i)
			{
				...
				if (transform != nullptr)
				{
					...
					asAudio::Update3D(&sound.soundinstance, instance3D);
				}
				if (sound.IsPlaying())
				{
					asAudio::Play(&sound.soundinstance);
				}
				else
				{
					asAudio::Stop(&sound.soundinstance);
				}
				if (!sound.IsLooped())
				{
					asAudio::ExitLoop(&sound.soundinstance);
				}
				asAudio::SetVolume(sound.volume, &sound.soundinstance);
			}
		}
		...
		{
			asArchive archive(fileName, true);
			if (archive.IsOpen())
			{
					...
					for (auto& subset : mesh.subsets)
					{
						for (size_t i = 0; i < subset.indexCount; i += 3)
						{
							...
							if (TriangleTests::Intersects(rayOrigin_local, rayDirection_local, p0, p1, p2, distance))
							{
								...
								distance = asMath::Distance(pos, rayOrigin);
