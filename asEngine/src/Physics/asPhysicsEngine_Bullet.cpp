...
namespace as
{
	...
	namespace asPhysicsEngine
	{
		...
		void Initialize()
		{
			...
			asBackLog::post("asPhysicsEngine_Bullet Initialized");
			AS_CORE_INFO("asPhysicsEngine_Bullet Initialized");
		}

		bool IsEnabled() { return ENABLED; }
		void SetEnabled(bool value) { ENABLED = value; }

		void AddRigidBody(Entity entity, asScene::RigidBodyPhysicsComponent& physicscomponent, const asScene::MeshComponent& mesh, const asScene::TransformComponent& transform)
		{
		...
		void AddSoftBody(Entity entity, asScene::SoftBodyPhysicsComponent& physicscomponent, const asScene::MeshComponent& mesh)
		{
		...
		)
		{
			...
			auto range = asProfiler::BeginRangeCPU("Physics");
			...
			asJobSystem::Dispatch(ctx, (uint32_t)rigidbodies.GetCount(), 256, [&](asJobDispatchArgs args) {
			...
			asJobSystem::Dispatch(ctx, (uint32_t)softbodies.GetCount(), 1, [&](asJobDispatchArgs args) {
			...
			asProfiler::EndRange(range); // Physics
		}
	}
}
