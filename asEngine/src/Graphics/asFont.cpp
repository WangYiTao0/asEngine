...
namespace as
{
	...
	{
		std::string			FONTPATH = asHelper::GetOriginalWorkingDirectory() + "../ASEngine/fonts/";
		...
		uint32_t WriteVertices(volatile FontVertex* vertexList, const std::wstring& text, asFontParams params, int style)
		{
			...
			for (auto& code : text)
			{
				...
				else
				{
					...
					const std::size_t vertexID = size_t(quadCount) * 4;
	...
	int asFont::AddFontStyle(const std::string& fontName)
	{
	...
	void asFont::SetText(const std::string& text)
	{
		as::asHelper::StringConvert(text, this->text);
	}
	void asFont::SetText(const std::wstring& text)
	{
		this->text = text;
	}
	std::wstring asFont::GetText() const
	{
		return text;
	}
	std::string asFont::GetTextA() const
	{
		std::string retval;
