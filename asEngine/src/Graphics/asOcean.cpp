...
#include "Graphics\GPUMapping\ShaderInterop_Ocean.h"
...
void asOcean::initHeightMap(const WeatherComponent& weather, XMFLOAT2* out_h0, float* out_omega)
{
	auto& params = weather.oceanParameters;
...
void asOcean::UpdateDisplacementMap(const WeatherComponent& weather, float time, CommandList cmd) const
{
	auto& params = weather.oceanParameters;
...
void asOcean::Render(const CameraComponent& camera, const WeatherComponent& weather, float time, CommandList cmd) const
{
	auto& params = weather.oceanParameters;
...
const Texture* asOcean::getGradientMap() const
{
	return &gradientMap;
}
