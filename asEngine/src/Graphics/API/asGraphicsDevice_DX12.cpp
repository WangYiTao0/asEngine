...
namespace as
{
	namespace asGraphics
	{
		...
		inline D3D12_CPU_DESCRIPTOR_HANDLE ToNativeHandle(asCPUHandle handle)
		{
		...
		constexpr D3D12_RESOURCE_STATES _ConvertImageLayout(IMAGE_LAYOUT value)
		{
			switch (value)
			{
			case asGraphics::IMAGE_LAYOUT_UNDEFINED:
			case asGraphics::IMAGE_LAYOUT_GENERAL:
				return D3D12_RESOURCE_STATE_COMMON;
			case asGraphics::IMAGE_LAYOUT_RENDERTARGET:
				return D3D12_RESOURCE_STATE_RENDER_TARGET;
			case asGraphics::IMAGE_LAYOUT_DEPTHSTENCIL:
				return D3D12_RESOURCE_STATE_DEPTH_WRITE;
			case asGraphics::IMAGE_LAYOUT_DEPTHSTENCIL_READONLY:
				return D3D12_RESOURCE_STATE_DEPTH_READ;
			case asGraphics::IMAGE_LAYOUT_SHADER_RESOURCE:
				return D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
			case asGraphics::IMAGE_LAYOUT_UNORDERED_ACCESS:
				return D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
			case asGraphics::IMAGE_LAYOUT_COPY_SRC:
				return D3D12_RESOURCE_STATE_COPY_SOURCE;
			case asGraphics::IMAGE_LAYOUT_COPY_DST:
		...
		constexpr D3D12_RESOURCE_STATES _ConvertBufferState(BUFFER_STATE value)
		{
			switch (value)
			{
			case asGraphics::BUFFER_STATE_GENERAL:
				return D3D12_RESOURCE_STATE_COMMON;
			case asGraphics::BUFFER_STATE_VERTEX_BUFFER:
				return D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
			case asGraphics::BUFFER_STATE_INDEX_BUFFER:
				return D3D12_RESOURCE_STATE_INDEX_BUFFER;
			case asGraphics::BUFFER_STATE_CONSTANT_BUFFER:
				return D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
			case asGraphics::BUFFER_STATE_INDIRECT_ARGUMENT:
				return D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT;
			case asGraphics::BUFFER_STATE_SHADER_RESOURCE:
				return D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
			case asGraphics::BUFFER_STATE_UNORDERED_ACCESS:
				return D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
			case asGraphics::BUFFER_STATE_COPY_SRC:
				return D3D12_RESOURCE_STATE_COPY_SOURCE;
			case asGraphics::BUFFER_STATE_COPY_DST:
				return D3D12_RESOURCE_STATE_COPY_DEST;
			}
		...
		void GraphicsDevice_DX12::DescriptorAllocator::free(asCPUHandle descriptorHandle)
		{
			if (descriptorHandle == AS_NULL_HANDLE)
			{
				return;
			}
		...
		GraphicsDevice_DX12::FrameResources::DescriptorTableFrameAllocator::DescriptorTableFrameAllocator(GraphicsDevice_DX12* device) : device(device)
		{
		...
		void GraphicsDevice_DX12::FrameResources::DescriptorTableFrameAllocator::create_or_bind_heaps_on_demand(CommandList cmd)
		{
			...
			if (currentheap_resource >= heaps_resource.size())
			{
				...
				HRESULT hr = device->device->CreateDescriptorHeap(&heap.heapDesc, __uuidof(ID3D12DescriptorHeap), (void**)&heap.heap_GPU);
				assert(SUCCEEDED(hr));
			...
			if (!heaps_bound)
			{
				ID3D12DescriptorHeap* heaps[] = {
					heaps_resource[currentheap_resource].heap_GPU, heaps_sampler[currentheap_sampler].heap_GPU
				};
			...
			for (int stage = 0; stage < SHADERSTAGE_COUNT; ++stage)
			{
				...
				if (table.dirty_samplers)
				{
					...
					for (uint32_t slot = 0; slot < GPU_SAMPLER_HEAP_COUNT; ++slot)
					{
						...
						if (sampler == nullptr || sampler->resource == AS_NULL_HANDLE)
						{
							continue;
						}
		...
		GraphicsDevice_DX12::FrameResources::ResourceFrameAllocator::ResourceFrameAllocator(GraphicsDevice_DX12* device, size_t size) : device(device)
		{
			...
			assert(SUCCEEDED(hr));
			buffer.resource = (asCPUHandle)resource;
		...
		{
			if (buffer.resource != AS_NULL_HANDLE)
			{
		...
		// Engine functions
		GraphicsDevice_DX12::GraphicsDevice_DX12(asPlatform::window_type window, bool fullscreen, bool debuglayer)
		{
			...
			if (FAILED(hr))
			{
				asHelper::messageBox("Failed to create a swapchain for the graphics device!", "Error!");
				assert(0);
				exit(1);
			}
		...
		{
			Texture result;
			result.resource = (asCPUHandle)GetFrameResources().backBuffer;
			result.type = GPUResource::GPU_RESOURCE_TYPE::TEXTURE;
			result.RTV = (asCPUHandle)GetFrameResources().backBufferRTV.ptr;
			return result;
		}
		...
		bool GraphicsDevice_DX12::CreateBuffer(const GPUBufferDesc* pDesc, const SubresourceData* pInitialData, GPUBuffer* pBuffer)
		{
			...
			assert(SUCCEEDED(hr));
			pBuffer->resource = (asCPUHandle)resource;
			destroylocker.lock();
		...
		bool GraphicsDevice_DX12::CreateTexture(const TextureDesc* pDesc, const SubresourceData* pInitialData, Texture* pTexture)
		{
			...
			assert(SUCCEEDED(hr));
			pTexture->resource = (asCPUHandle)resource;
			destroylocker.lock();
		...
		bool GraphicsDevice_DX12::CreateQuery(const GPUQueryDesc* pDesc, GPUQuery* pQuery)
		{
			...
			case GPU_QUERY_TYPE_TIMESTAMP:
				...
				{
					pQuery->resource = (asCPUHandle)query_index;
					hr = S_OK;
				}
				else
				{
		...
		bool GraphicsDevice_DX12::CreatePipelineState(const PipelineStateDesc* pDesc, PipelineState* pso)
		{
			...
			pso->hash = 0;
			asHelper::hash_combine(pso->hash, pDesc->vs);
			asHelper::hash_combine(pso->hash, pDesc->ps);
			asHelper::hash_combine(pso->hash, pDesc->hs);
			asHelper::hash_combine(pso->hash, pDesc->ds);
			asHelper::hash_combine(pso->hash, pDesc->gs);
			asHelper::hash_combine(pso->hash, pDesc->il);
			asHelper::hash_combine(pso->hash, pDesc->rs);
			asHelper::hash_combine(pso->hash, pDesc->bs);
			asHelper::hash_combine(pso->hash, pDesc->dss);
			asHelper::hash_combine(pso->hash, pDesc->pt);
			asHelper::hash_combine(pso->hash, pDesc->sampleMask);
		...
		bool GraphicsDevice_DX12::CreateRenderPass(const RenderPassDesc* pDesc, RenderPass* renderpass)
		{
			...
			renderpass->hash = 0;
			asHelper::hash_combine(renderpass->hash, pDesc->numAttachments);
			for (uint32_t i = 0; i < pDesc->numAttachments; ++i)
			{
				asHelper::hash_combine(renderpass->hash, pDesc->attachments[i].texture->desc.Format);
				asHelper::hash_combine(renderpass->hash, pDesc->attachments[i].texture->desc.SampleCount);
			}
		...
		int GraphicsDevice_DX12::CreateSubresource(Texture* texture, SUBRESOURCE_TYPE type, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
		{
			...
			case asGraphics::SRV:
			{
				...
				asCPUHandle srv = ResourceAllocator->allocate();
				device->CreateShaderResourceView((ID3D12Resource*)texture->resource, &srv_desc, ToNativeHandle(srv));

				if (texture->SRV == AS_NULL_HANDLE)
				{
					texture->SRV = (asCPUHandle)srv;
					return -1;
				}
				texture->subresourceSRVs.push_back((asCPUHandle)srv);
			...
			case asGraphics::UAV:
			{
				...
				asCPUHandle uav = ResourceAllocator->allocate();
				device->CreateUnorderedAccessView((ID3D12Resource*)texture->resource, nullptr, &uav_desc, ToNativeHandle(uav));

				if (texture->UAV == AS_NULL_HANDLE)
				{
					texture->UAV = (asCPUHandle)uav;
					return -1;
				}
				texture->subresourceUAVs.push_back((asCPUHandle)uav);
			...
			case asGraphics::RTV:
			{
				...
				asCPUHandle rtv = RTAllocator->allocate();
				device->CreateRenderTargetView((ID3D12Resource*)texture->resource, &rtv_desc, ToNativeHandle(rtv));

				if (texture->RTV == AS_NULL_HANDLE)
				{
					texture->RTV = (asCPUHandle)rtv;
					return -1;
				}
				texture->subresourceRTVs.push_back((asCPUHandle)rtv);
			...
			case asGraphics::DSV:
			{
				...
				asCPUHandle dsv = DSAllocator->allocate();
				device->CreateDepthStencilView((ID3D12Resource*)texture->resource, &dsv_desc, ToNativeHandle(dsv));

				if (texture->DSV == AS_NULL_HANDLE)
				{
					texture->DSV = (asCPUHandle)dsv;
					return -1;
				}
				texture->subresourceDSVs.push_back((asCPUHandle)dsv);
		...
		{
			if (pShader->stage == CS && pShader->resource != AS_NULL_HANDLE)
			{
			...
			// Execute deferred command lists:
			{
				...
				while (active_commandlists.pop_front(cmd))
				{
					...
					for (auto& x : pipelines_worker[cmd])
					{
						...
						{
							DeferredDestroy({ DestroyItem::PIPELINE, FRAMECOUNT, (asCPUHandle)x.second });
						}
					}
		...
		CommandList GraphicsDevice_DX12::BeginCommandList()
		{
			...
			if (!free_commandlists.pop_front(cmd))
			{
				...
				for (uint32_t fr = 0; fr < BACKBUFFER_COUNT; ++fr)
				{
					...
					frames[fr].descriptors[cmd] = new FrameResources::DescriptorTableFrameAllocator(this);
		...
		void GraphicsDevice_DX12::BindPipelineState(const PipelineState* pso, CommandList cmd)
		{
			...
			asHelper::hash_combine(pipeline_hash, pso->hash);
			if (active_renderpass[cmd] != nullptr)
			{
				asHelper::hash_combine(pipeline_hash, active_renderpass[cmd]->hash);
			}
