...
#include "Graphics/GPUMapping/ShaderInterop_Vulkan.h"
...
namespace as
{
	namespace asGraphics
	{
		...
		constexpr VkStencilOp _ConvertStencilOp(STENCIL_OP value)
		{
			switch (value)
			{
			case asGraphics::STENCIL_OP_KEEP:
				return VK_STENCIL_OP_KEEP;
				break;
			case asGraphics::STENCIL_OP_ZERO:
				return VK_STENCIL_OP_ZERO;
				break;
			case asGraphics::STENCIL_OP_REPLACE:
				return VK_STENCIL_OP_REPLACE;
				break;
			case asGraphics::STENCIL_OP_INCR_SAT:
				return VK_STENCIL_OP_INCREMENT_AND_CLAMP;
				break;
			case asGraphics::STENCIL_OP_DECR_SAT:
				return VK_STENCIL_OP_DECREMENT_AND_CLAMP;
				break;
			case asGraphics::STENCIL_OP_INVERT:
				return VK_STENCIL_OP_INVERT;
				break;
			case asGraphics::STENCIL_OP_INCR:
				return VK_STENCIL_OP_INCREMENT_AND_WRAP;
				break;
			case asGraphics::STENCIL_OP_DECR:
		...
		GraphicsDevice_Vulkan::FrameResources::ResourceFrameAllocator::ResourceFrameAllocator(GraphicsDevice_Vulkan* device, size_t size) : device(device)
		{
			...
			this->buffer.resource = (asCPUHandle)buffer;
				...
				if (table.dirty)
				{
					...
					// Now fill in only the used descriptors one by one:
					{
						...
						for (int slot = 0; slot < GPU_RESOURCE_HEAP_SRV_COUNT; ++slot)
						{
							...
							asCPUHandle SRV = subresource < 0 ? resource->SRV : resource->subresourceSRVs[subresource];
							...
							else if (resource->IsBuffer())
							{
								...
								if (buffer->desc.Format == FORMAT_UNKNOWN && buffer->resource != AS_NULL_HANDLE)
								{
						...
						for (int slot = 0; slot < GPU_RESOURCE_HEAP_UAV_COUNT; ++slot)
						{
							...
							asCPUHandle UAV = subresource < 0 ? resource->UAV : resource->subresourceUAVs[subresource];
		...
		// Engine functions
		GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(asPlatform::window_type window, bool fullscreen, bool debuglayer)
		{
			...
			if (enableValidationLayers && !checkValidationLayerSupport()) {
				asHelper::messageBox("Vulkan validation layer requested but not available!");
...
#error WICKEDENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
			...
			// Enumerating and creating devices:
			{
				...
				if (deviceCount == 0) {
					asHelper::messageBox("failed to find GPUs with Vulkan support!");
					assert(0);
				}
				...
				if (physicalDevice == VK_NULL_HANDLE) {
					asHelper::messageBox("failed to find a suitable GPU!");
					assert(0);
				}
		...
		bool GraphicsDevice_Vulkan::CreateBuffer(const GPUBufferDesc* pDesc, const SubresourceData* pInitialData, GPUBuffer* pBuffer)
		{
			...
			assert(res == VK_SUCCESS);
			pBuffer->resource = (asCPUHandle)buffer;
			destroylocker.lock();
		...
		bool GraphicsDevice_Vulkan::CreateTexture(const TextureDesc* pDesc, const SubresourceData* pInitialData, Texture* pTexture)
		{
			...
			assert(res == VK_SUCCESS);
			pTexture->resource = (asCPUHandle)image;
			destroylocker.lock();
		...
		bool GraphicsDevice_Vulkan::CreateRenderPass(const RenderPassDesc* pDesc, RenderPass* renderpass)
		{
			...
			renderpass->renderpass = (asCPUHandle)renderpass_handle;
			renderpass->framebuffer = (asCPUHandle)framebuffer_handle;
		...
		void GraphicsDevice_Vulkan::DestroyTexture(Texture* pTexture)
		{
			...
			pTexture->resource = AS_NULL_HANDLE;
		...
		void GraphicsDevice_Vulkan::DestroyRenderPass(RenderPass* renderpass)
		{
			...
			renderpass->renderpass = AS_NULL_HANDLE;
			DeferredDestroy({ DestroyItem::FRAMEBUFFER, renderpass->framebuffer });
			renderpass->framebuffer = AS_NULL_HANDLE;
		}
