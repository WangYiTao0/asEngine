...
namespace as
{
	namespace asRawInput
	{
		asAllocators::LinearAllocator allocator;
		std::vector<uint8_t*> input_messages;

		asInput::KeyboardState keyboard;
		asInput::MouseState mouse;
		...
		void ParseRawInputBlock(const RAWINPUT& raw)
		{
			...
			else if (raw.header.dwType == RIM_TYPEHID)
			{
				...
				if (!internal_controller.is_xinput) // xinput enabled controller will be handled by xinput API
				{
					...
					asInput::ControllerState& controller = internal_controller.state;

					for (ULONG i = 0; i < numberOfButtons; i++)
					{
						int button = usage[i] - pButtonCaps->Range.UsageMin;
						controller.buttons |= 1 << (button + asInput::GAMEPAD_BUTTON_1 - asInput::GAMEPAD_RANGE_START - 1);
					}
					...
					for (USHORT i = 0; i < Caps.NumberInputValueCaps; i++)
					{
						...
						case 0x39:  // Hat Switch
						{
							...
							case POV_UPRIGHT:
							case POV_LEFTUP:
								controller.buttons |= 1 << (asInput::GAMEPAD_BUTTON_UP - asInput::GAMEPAD_RANGE_START - 1);
								break;
							}
							switch (pov)
							{
							...
							case POV_UPRIGHT:
							case POV_RIGHTDOWN:
								controller.buttons |= 1 << (asInput::GAMEPAD_BUTTON_RIGHT - asInput::GAMEPAD_RANGE_START - 1);
								break;
							}
							switch (pov)
							{
							...
							case POV_RIGHTDOWN:
							case POV_DOWNLEFT:
								controller.buttons |= 1 << (asInput::GAMEPAD_BUTTON_DOWN - asInput::GAMEPAD_RANGE_START - 1);
								break;
							}
							switch (pov)
							{
							...
							case POV_DOWNLEFT:
							case POV_LEFTUP:
								controller.buttons |= 1 << (asInput::GAMEPAD_BUTTON_LEFT - asInput::GAMEPAD_RANGE_START - 1);
								break;
							}
						}
						break;
		...
		void Update()
		{
			keyboard = asInput::KeyboardState();
			mouse = asInput::MouseState();
			for (auto& internal_controller : controllers)
			{
				internal_controller.state = asInput::ControllerState();
			}
		...
		void GetKeyboardState(asInput::KeyboardState* state)
		{
			*state = keyboard;
		}

		void GetMouseState(asInput::MouseState* state)
		{
			*state = mouse;
		}
