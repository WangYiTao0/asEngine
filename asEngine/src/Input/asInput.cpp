...
#define KEY_UP(vk_code) (!KEY_DOWN(vk_code))
		...
		void Initialize()
		{
			asRawInput::Initialize();

			asBackLog::post("asInput Initialized");
		...
		void Update()
		{
			...
			auto range = asProfiler::BeginRangeCPU("Input");

			asXInput::Update();
			asRawInput::Update();

			asRawInput::GetMouseState(&mouse); // currently only the relative data can be used from this
			asRawInput::GetKeyboardState(&keyboard); // it contains pressed buttons as "keyboard/typewriter" like, so no continuous presses

			// Check if low-level XINPUT controller is not registered for playerindex slot and register:
			for (int i = 0; i < asXInput::GetMaxControllerCount(); ++i)
			{
				if (asXInput::GetControllerState(nullptr, i))
				{
					int slot = -1;
			...
			for (int i = 0; i < asRawInput::GetMaxControllerCount(); ++i)
			{
				if (asRawInput::GetControllerState(nullptr, i))
				{
					int slot = -1;
					for (int j = 0; j < (int)controllers.size(); ++j)
					{
			...
			for (auto& controller : controllers)
			{
				...
				case Controller::XINPUT:
					connected = asXInput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
				case Controller::RAWINPUT:
					connected = asRawInput::GetControllerState(&controller.state, controller.deviceIndex);
					break;
				}
		...
		bool Down(BUTTON button, int playerindex)
		{
			...
			else if (playerindex == 0) // keyboard or mouse
			{
				...
				switch (button)
				{
				case asInput::MOUSE_BUTTON_LEFT:
					keycode = VK_LBUTTON;
					break;
				case asInput::MOUSE_BUTTON_RIGHT:
					keycode = VK_RBUTTON;
					break;
				case asInput::MOUSE_BUTTON_MIDDLE:
					keycode = VK_MBUTTON;
					break;
				case asInput::KEYBOARD_BUTTON_UP:
					keycode = VK_UP;
					break;
				case asInput::KEYBOARD_BUTTON_DOWN:
					keycode = VK_DOWN;
					break;
				case asInput::KEYBOARD_BUTTON_LEFT:
					keycode = VK_LEFT;
					break;
				case asInput::KEYBOARD_BUTTON_RIGHT:
					keycode = VK_RIGHT;
					break;
				case asInput::KEYBOARD_BUTTON_SPACE:
					keycode = VK_SPACE;
					break;
				case asInput::KEYBOARD_BUTTON_RSHIFT:
					keycode = VK_RSHIFT;
					break;
				case asInput::KEYBOARD_BUTTON_LSHIFT:
					keycode = VK_LSHIFT;
					break;
				case asInput::KEYBOARD_BUTTON_F1:
					keycode = VK_F1;
					break;
				case asInput::KEYBOARD_BUTTON_F2:
					keycode = VK_F2;
					break;
				case asInput::KEYBOARD_BUTTON_F3:
					keycode = VK_F3;
					break;
				case asInput::KEYBOARD_BUTTON_F4:
					keycode = VK_F4;
					break;
				case asInput::KEYBOARD_BUTTON_F5:
					keycode = VK_F5;
					break;
				case asInput::KEYBOARD_BUTTON_F6:
					keycode = VK_F6;
					break;
				case asInput::KEYBOARD_BUTTON_F7:
					keycode = VK_F7;
					break;
				case asInput::KEYBOARD_BUTTON_F8:
					keycode = VK_F8;
					break;
				case asInput::KEYBOARD_BUTTON_F9:
					keycode = VK_F9;
					break;
				case asInput::KEYBOARD_BUTTON_F10:
					keycode = VK_F10;
					break;
				case asInput::KEYBOARD_BUTTON_F11:
					keycode = VK_F11;
					break;
				case asInput::KEYBOARD_BUTTON_F12:
					keycode = VK_F12;
					break;
				case asInput::KEYBOARD_BUTTON_ENTER:
					keycode = VK_RETURN;
					break;
				case asInput::KEYBOARD_BUTTON_ESCAPE:
					keycode = VK_ESCAPE;
					break;
				case asInput::KEYBOARD_BUTTON_HOME:
					keycode = VK_HOME;
					break;
				case asInput::KEYBOARD_BUTTON_LCONTROL:
					keycode = VK_LCONTROL;
					break;
				case asInput::KEYBOARD_BUTTON_RCONTROL:
					keycode = VK_RCONTROL;
					break;
				case asInput::KEYBOARD_BUTTON_DELETE:
					keycode = VK_DELETE;
					break;
				case asInput::KEYBOARD_BUTTON_BACKSPACE:
					keycode = VK_BACK;
					break;
				case asInput::KEYBOARD_BUTTON_PAGEDOWN:
					keycode = VK_NEXT;
					break;
				case asInput::KEYBOARD_BUTTON_PAGEUP:
					keycode = VK_PRIOR;
					break;
				}
		...
		{
			MouseState state;
			asRawInput::GetMouseState(&state);

#ifndef WINSTORE_SUPPORT
			POINT p;
			GetCursorPos(&p);
			ScreenToClient(asPlatform::GetWindow(), &p);
		...
		void SetPointer(const XMFLOAT4& props)
		{
			...
			p.x = (LONG)props.x;
			p.y = (LONG)props.y;
			ClientToScreen(asPlatform::GetWindow(), &p);
		...
		void SetControllerFeedback(const ControllerFeedback& data, int playerindex)
		{
			if (playerindex < (int)controllers.size())
			{
				...
				{
					asXInput::SetControllerFeedback(data, controller.deviceIndex);
				}
				else if (controller.deviceType == Controller::RAWINPUT)
				{
					asRawInput::SetControllerFeedback(data, controller.deviceIndex);
				}
			}
		}
