...
#include "../3rdPart/tiny_obj_loader.h"
...
namespace as
{
	...
	void ImportModel_OBJ(const std::string& fileName, Scene& scene)
	{
		...
		if (!obj_errors.empty())
		{
			asBackLog::post(obj_errors.c_str());
			AS_CORE_ERROR(obj_errors.c_str());
		}
		...
		if (success)
		{
			...
			for (auto& obj_material : obj_materials)
			{
				...
				if (!material.surfaceMapName.empty())
				{
					material.surfaceMap = asResourceManager::Load(directory + material.surfaceMapName);
				}
				if (!material.baseColorMapName.empty())
				{
					material.baseColorMap = asResourceManager::Load(directory + material.baseColorMapName);
				}
				if (!material.normalMapName.empty())
				{
					material.normalMap = asResourceManager::Load(directory + material.normalMapName);
				}
				if (!material.displacementMapName.empty())
				{
					material.displacementMap = asResourceManager::Load(directory + material.displacementMapName);
				}
			...
			for (auto& shape : obj_shapes)
			{
				...
				for (size_t i = 0; i < shape.mesh.indices.size(); i += 3)
				{
					...
					for (auto& index : reordered_indices)
					{
						...
						size_t vertexHash = 0;
						asHelper::hash_combine(vertexHash, index.vertex_index);
						asHelper::hash_combine(vertexHash, index.normal_index);
						asHelper::hash_combine(vertexHash, index.texcoord_index);
						asHelper::hash_combine(vertexHash, materialIndex);
		...
		{
			asHelper::messageBox("OBJ import failed! Check backlog for errors!", "Error!");
		}
	}
}