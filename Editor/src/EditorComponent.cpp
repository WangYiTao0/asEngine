...
namespace as
{
	...
	void EditorLoadingScreen::Load()
	{
		...
		sprite = asSprite("../images/logo_small.png");
		sprite.anim.opa = 1;
	...
	void EditorComponent::Load()
	{
		...
		asJobSystem::Execute(ctx, [this] { pointLightTex = asResourceManager::Load("images/pointlight.dds"); });
		asJobSystem::Execute(ctx, [this] { spotLightTex = asResourceManager::Load("images/spotlight.dds"); });
		asJobSystem::Execute(ctx, [this] { dirLightTex = asResourceManager::Load("images/directional_light.dds"); });
		asJobSystem::Execute(ctx, [this] { areaLightTex = asResourceManager::Load("images/arealight.dds"); });
		asJobSystem::Execute(ctx, [this] { decalTex = asResourceManager::Load("images/decal.dds"); });
		asJobSystem::Execute(ctx, [this] { forceFieldTex = asResourceManager::Load("images/forcefield.dds"); });
		asJobSystem::Execute(ctx, [this] { emitterTex = asResourceManager::Load("images/emitter.dds"); });
		asJobSystem::Execute(ctx, [this] { hairTex = asResourceManager::Load("images/hair.dds"); });
		asJobSystem::Execute(ctx, [this] { cameraTex = asResourceManager::Load("images/camera.dds"); });
		asJobSystem::Execute(ctx, [this] { armatureTex = asResourceManager::Load("images/armature.dds"); });
		asJobSystem::Execute(ctx, [this] { soundTex = asResourceManager::Load("images/sound.dds"); });
				...
				if (object != nullptr)
				{
					if (object->GetRenderTypes() & RENDERTYPE_WATER)
					{
						if (asInput::Down(asInput::MOUSE_BUTTON_LEFT))
						{
							...
							asRenderer::PutWaterRipple(asHelper::GetOriginalWorkingDirectory() + "images/ripple.png", hovered.position);
						}
					}
					else
					{
						if (asInput::Press(asInput::MOUSE_BUTTON_LEFT))
						{
							...
							else
							{
								...
								Entity entity = scene.Entity_CreateDecal("editorDecal", asHelper::GetOriginalWorkingDirectory() + (decalselector == 0 ? "images/leaf.dds" : "images/blood1.png"));
