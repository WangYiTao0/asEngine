...
std::shared_ptr<asResource> RegisterTexture(tinygltf::Image* image, const string& type_name)
{
	// We will load the texture2d by hand here and register to the resource manager (if it was not already registered)
	if (!asResourceManager::Contains(asHashString(image->uri)))
	{
		...
		if (!image->image.empty())
		{
			...
			if (device->CreateTexture(&desc, InitData.data(), tex))
			{
				...
				if (tex != nullptr)
				{
					asRenderer::AddDeferredMIPGen(tex);
					...
					if (image->uri.empty())
					{
						...
						do {
							ss.str("");
							ss << "gltfimport_" << type_name << "_" << asRandom::getRandom(INT_MAX) << ".png";
						} while (asHelper::FileExists(ss.str())); // this is to avoid overwriting an existing exported image
						image->uri = ss.str();
						bool success = asHelper::saveTextureToFile(image->image, desc, ss.str());
						assert(success);
					}

					// We loaded the texture2d, so register to the resource manager to be retrieved later:
					return asResourceManager::Register(image->uri, tex, asResource::IMAGE);
				}
			}
			else
			{
...
void LoadNode(int nodeIndex, Entity parent, LoaderState& state)
{
	...
	else if (node.camera >= 0)
	{
		...
		entity = scene.Entity_CreateCamera(node.name, (float)asRenderer::GetInternalResolution().x, (float)asRenderer::GetInternalResolution().y, 0.1f, 800);
	}
...
void as::ImportModel_GLTF(const std::string& fileName, Scene& scene)
{
	string directory, name;
	asHelper::SplitPath(fileName, directory, name);
	string extension = asHelper::toUpper(asHelper::GetExtensionFromFileName(name));
	asHelper::RemoveExtensionFromFileName(name);
	...
	if (!ret) {
		asHelper::messageBox(err, "GLTF error!");
	}
			...
			for (auto& attr : prim.attributes)
			{
				...
				else if (!attr_name.compare("COLOR_0"))
				{
					...
					for (size_t i = 0; i < vertexCount; ++i)
					{
						...
						uint32_t rgba = asMath::CompressColor(color);
