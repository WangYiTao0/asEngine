...
namespace as
{
	...
	static Atlas_Dim GenerateMeshAtlas(MeshComponent& meshcomponent, uint32_t resolution)
	{
		...
		// Prepare mesh to be processed by xatlas:
		{
			...
			if (error != xatlas::AddMeshError::Success) {
				asHelper::messageBox(xatlas::StringForEnum(error), "Adding mesh to xatlas failed!");
				return dim;
			}
		}
	...
	ObjectWindow::ObjectWindow(EditorComponent* editor) : editor(editor)
	{
		...
		objectWindow = new asWindow(GUI, "Object Window");
		...
		renderableCheckBox->OnClick([&](asEventArgs args) {
			ObjectComponent* object = asScene::GetScene().objects.GetComponent(entity);
			if (object != nullptr)
			{
		...
		ditherSlider = new asSlider(0, 1, 0, 1000, "Dither: ");
		...
		ditherSlider->OnSlide([&](asEventArgs args) {
			ObjectComponent* object = asScene::GetScene().objects.GetComponent(entity);
			if (object != nullptr)
			{
		...
		cascadeMaskSlider = new asSlider(0, 3, 0, 3, "Cascade Mask: ");
		...
		cascadeMaskSlider->OnSlide([&](asEventArgs args) {
			ObjectComponent* object = asScene::GetScene().objects.GetComponent(entity);
			if (object != nullptr)
			{
		...
		physicsLabel = new asLabel("PHYSICSLABEL");
		...
		rigidBodyCheckBox = new asCheckBox("Rigid Body Physics: ");
		...
		rigidBodyCheckBox->OnClick([&](asEventArgs args)
			{
		...
		kinematicCheckBox = new asCheckBox("Kinematic: ");
		...
		kinematicCheckBox->OnClick([&](asEventArgs args) {
			RigidBodyPhysicsComponent* physicscomponent = asScene::GetScene().rigidbodies.GetComponent(entity);
			if (physicscomponent != nullptr)
			{
		...
		disabledeactivationCheckBox = new asCheckBox("Disable Deactivation: ");
		...
		disabledeactivationCheckBox->OnClick([&](asEventArgs args) {
			RigidBodyPhysicsComponent* physicscomponent = asScene::GetScene().rigidbodies.GetComponent(entity);
			if (physicscomponent != nullptr)
			{
		...
		collisionShapeComboBox = new asComboBox("Collision Shape:");
		...
		collisionShapeComboBox->OnSelect([&](asEventArgs args)
			{
				RigidBodyPhysicsComponent* physicscomponent = asScene::GetScene().rigidbodies.GetComponent(entity);
				if (physicscomponent != nullptr)
				{
	...
	void ObjectWindow::SetEntity(Entity entity)
	{
		...
		if (object != nullptr)
		{
			...
			colorPicker->SetPickColor(asColor::fromFloat4(object->color));
